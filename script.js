...
function bukaBayarTagihan(){
  ...
  document.getElementById('bayar-nominal').textContent = document.getElementById('tagihan-total').textContent;
  ...
  db.collection('tagihan').where('uid', '==', auth.currentUser.uid).get().then(snap => {
    ...
    const docs = urutkanBerdasarkanTanggal(snap.docs, 'dibuatPada');
    const tagihanAktif = docs.find(d => d.data().status !== 'Lunas') || docs[0];
    if(!tagihanAktif || tagihanAktif.data().status === 'Lunas'){
      errorEl.textContent = 'Tagihan Anda sudah lunas.';
      return;
    }
    const buatInvoice = cloudFunctions.httpsCallable('buatTagihanXendit');
    return buatInvoice({ tagihanId: tagihanAktif.id }).then(result => {
